 ...
 */
@Slf4j
class CustomHtmlReportCreator extends AbstractHtmlCreator<SpecData>
        implements IReportCreator {

    final CustomHtmlReportAggregator reportAggregator
    ...
    CustomHtmlReportCreator() {
        reportAggregator = new CustomHtmlReportAggregator()
    }

    CustomHtmlReportCreator( CustomHtmlReportAggregator reportAggregator ) {
    ...
    @Override
    String cssDefaultName() { 'spock-feature-report.css' }
            ...
            for ( FeatureInfo feature in data.info.allFeaturesInExecutionOrder ) {
                ...
                if ( run && Utils.isUnrolled( feature ) ) {
                    run.failuresByIteration.eachWithIndex { iteration, problems, int index ->
                        ...
                        li {
                            a( href: "#${URLEncoder.encode(name, 'UTF-8')}", 'class': "feature-toc-$cssClass", name )
                        }
                    }
                } else {
                    ...
                    li {
                        a( href: "#${URLEncoder.encode(feature.name, 'UTF-8')}", 'class': "feature-toc-$cssClass", feature.name )
                    }
                }
            }
        }
    ...
    private void writeFeatureDescription( MarkupBuilder builder, String name,
        ...
        builder.tr {
            td( colspan: '10' ) {
                div( 'class': 'feature-description' + cssClass, id: URLEncoder.encode(name, 'UTF-8') ) {
                    span name
